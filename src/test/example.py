...
def learn_and_reconstruct():
    ...
    NDL = NetDictLearner(G=G, n_components=25, MCMC_iterations=20)
    NDL.train_dict(verbose=True)
    G_recons = NDL.reconstruct(recons_iter=5000)
...
def additive_corruption():
    ...
    G_corrupt_recons = NDL_corrupt.reconstruct(recons_iter=1000, return_weighted=True)
    F, T, auc, fpr, tpr, thresholds = utils.auc_roc(G, G_corrupt, G_corrupt_recons, noise_type="positive")
    print(auc)
...
def subtractive_corruption():
    ...
    NDL_corrupt = NetDictLearner(G=G_corrupt, n_components=25, MCMC_iterations=2)

    NDL_corrupt.train_dict()
    G_corrupt_recons = NDL_corrupt.reconstruct(recons_iter=5000, return_weighted=True)
    F, T, auc, fpr, tpr, thresholds = utils.auc_roc(G, G_corrupt, G_corrupt_recons, noise_type="negative")
    print(auc)
